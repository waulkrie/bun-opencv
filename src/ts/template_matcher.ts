...
export class TemplateMatcher {
  ...
  static imread(filename: string, flags: number): Mat {
    const matPtr = symbols.cv_imread(ptr(Buffer.from(filename + '\0')), flags);
    return new Mat(matPtr);
  }

  // Template matching wrapper
  static matchTemplate(image: Mat, templ: Mat, method: number): Mat {
    const resultPtr = symbols.cv_match_template(image.ptr, templ.ptr, method);
  ...
  static getSize(mat: Mat): { width: number, height: number } {
    ...
    symbols.cv_get_size(mat.ptr, ptr(sizeBuffer), ptr(sizeBuffer.subarray(1)));
    return {
      width: sizeBuffer[0],
      height: sizeBuffer[1]
    };
  ...
  static getMatData(mat: Mat, size: number): Float32Array {
    ...
    symbols.cv_get_mat_data(mat.ptr, ptr(buffer));
    return buffer;
  }
} 